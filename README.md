...
### Ops (Phase 6): container + logging + probes + admin auth

```bash
docker compose up -d      # postgres + api (image tự chứa: model + .npy bake lúc build)
curl localhost:8000/health   # liveness  — 200 khi process sống (kể cả đang warmup)
curl localhost:8000/ready    # readiness — 200 khi index build xong + DB chạm được
```

Env bắt buộc khi deploy (qua environment/secrets manager, KHÔNG vào image —
`.dockerignore` chặn `.env`): `DATABASE_URL`, `ADMIN_TOKEN` (bắt buộc đặt để bật
ingestion — không đặt là endpoint admin KHOÁ 503, khác search mock-mode),
`AWS_LOCATION_API_KEY` + `AWS_DEFAULT_REGION` (verify). Tuỳ chọn: `LOG_LEVEL`
(default INFO). Log là JSON ra stdout (12-factor): mỗi request có
`method/path/status/latency_ms` (không log query string — lat/lon người dùng);
ingest log counts + `unverified_ratio`; reload log `from_version→to_version`.
Admin auth: `Authorization: Bearer $ADMIN_TOKEN`, so sánh constant-time.

**Đa-instance (Phase 5):** ingest bump `data_version` trong bảng `meta` **cùng
transaction** với upsert (rollback → version không tăng); mỗi serve instance chạy
poller daemon (chu kỳ `search.version_poll_seconds`, mặc định 10s) đọc version từ
Postgres — thấy mới hơn bản đang giữ → reload qua atomic swap. Instance nhận batch
reindex ngay không chờ poll; các instance khác bắt kịp eventual (trễ ≤ 1 chu kỳ poll).

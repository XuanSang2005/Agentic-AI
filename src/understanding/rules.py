...
được đọc ở code path planner nào nữa (typo_fix vẫn dùng làm vocab) — file giữ
lại trong repo để tham khảo.

City KHÔNG hardcode: danh sách city sinh từ DATA (load_pois — nguồn Postgres/xlsx),
alias khai trong config/city_aliases.yaml — thêm POI thành phố mới là tự nhận.
"""
...
# --- Wordlist từ config/settings.yaml (understanding.popularity_cues) — non-dev
# thêm từ không phải sửa code. Từ ở dạng ĐÃ BỎ DẤU.
...
def _collect_city_spans(norm: str, plan: QueryPlan) -> list[tuple[int, int]]:
    ...
    City rules là data-driven (mỗi city nhiều pattern surface) — lấy span
    LEFTMOST (tie-break surface dài hơn) của đúng city đã match, y hệt hành vi
    regex alternation 1-pattern-1-city cũ.
    """
    if plan.city is None:
        return []
    pats = next((pats for city, pats in _city_rules() if city == plan.city), ())
    ...
    for pat in pats:
        ...
        if m is None:
            continue
        s, e = m.span()
        if best is None or (s, -(e - s)) < (best[0], -(best[1] - best[0])):
            best = (s, e)
...
def extract_plan(query: str, attr_index=None, joint_index=None, column_anchor=None) -> QueryPlan:
    ...
    if attr_index is not None:
        ...
        if landmark_span is not None:
            ...
            pre_window = norm[max(0, landmark_span[0] - _NEAR_CUE_WINDOW):landmark_span[0]]
            cue_m = _NEAR_CUE.search(pre_window)
            if cue_m:
                offset = max(0, landmark_span[0] - _NEAR_CUE_WINDOW)

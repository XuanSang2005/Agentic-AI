"""Dense retrieval: multilingual-e5-small, Qdrant local mode (embedded, offline).
...
v2 (dynamic-vector-attributes): DenseRetriever chạy vector search qua Qdrant
local mode + hybrid pre-filtering theo QueryPlan (city/district/category).
AttributeIndex / JointMetadataIndex / ColumnAnchorIndex: index vector THUẦN
numpy trên metadata của dataset (unique attributes/categories + topic anchors)
— trái tim của query understanding không-LLM, học trực tiếp từ data.
"""
...
class DenseRetriever:
    ...
    reuse_embeddings / model (reindex sau ingestion): map document→vector và
    model instance từ index CŨ. Có reuse → chỉ encode doc CHƯA có vector (POI mới),
    ...
    def __init__(self, pois: list[POI], model_name: str = config.EMBEDDING_MODEL,
        ...
        try:
            ...
            # Fallback sang in-memory nếu thư mục bị lock bởi tiến trình khác
            # (vd: uvicorn đang giữ, hoặc index cũ còn sống trong lúc reindex)
            self._client = QdrantClient(location=":memory:")

        # RECREATE collection mỗi lần build: index phải phản chiếu ĐÚNG corpus
        # hiện tại — create-if-missing + upsert sẽ để sót point cũ khi POI bị
        # gỡ/đổi giữa 2 lần build (reindex sau ingestion, storage cũ trên đĩa).
        if self._client.collection_exists(self._collection_name):
            self._client.delete_collection(self._collection_name)
        self._client.create_collection(
            collection_name=self._collection_name,
            vectors_config=VectorParams(size=int(self._doc_emb.shape[1]),
